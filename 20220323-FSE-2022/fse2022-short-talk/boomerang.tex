...
\section{A Very Short Introduction to Sandwich Distinguishers}
\sectionheader[\huge\color{tug}\faBook]{Sandwich Distinguishers}
%
%\begin{frame}{Exploiting Two Short Strong Differentials}
%\begin{figure}
%\centering
%\begin{tikzpicture}
...
\begin{figure}
\only<1->{
\begin{subfigure}{0.55\textheight}
\centering
...
\only<2>{
	$
	\Pr(\Delta_{1} \xrightarrow{E_{0}} \Delta_{2}) = p; \quad
    \textcolor{white}{\Pr(\nabla_{3} \xrightarrow{E_{1}} \nabla_{4}) = q}
	$
}
\only<3>{
	$
...
\only<4>{
	$
	\Pr(P_{3} \oplus P_{4} = \textcolor{red}{\Delta_{1}}) \approx p^{2}\times r \times q^{2} \quad ~ ~ ~ ~ ~ ~ \quad
	$
}
\only<5>{
$
...
\begin{frame}{Instantiating Truncated Trails with Concrete Differentials}
	\begin{itemize}
		...
		\pause
	\item[\faWarning] \textbf{\textcolor{lightred}{Our distinguishers are \textbf{not} relied on differential characteristics for}} $E_{0}, E_{1}, E_{m}$
...
%\sectionheader[\huge\color{tug}\faAlignJustify]{Our New Tools in BCT Framework}
...
%\begin{frame}{22-round Practical Sandwich Distinguisher for SKINNY-64-192}
%\centering
%%\only<1>{%
%%\begin{figure}
...
%\only<1>{%
%\begin{figure}
%\includegraphics[width=0.57\textwidth]{./figures/rumi_64_192_bmd3_2.pdf}
%\end{figure}
%}
%\end{frame}
