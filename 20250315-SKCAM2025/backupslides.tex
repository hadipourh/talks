%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Universal Bound for Data Complexity}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Universal Bound for Data Complexity - I}
\begin{theorem}[Data Complexity]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Universal Bound for Data Complexity -- II}
\setlength{\columnsep}{1em} % Adjust column spacing
\begin{columns}[t] % Align columns at the top
  % First column for the itemize
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Differential Attacks \cite{crypto_BihamS90}}
\vspace{-0.4cm}
...
\begin{algorithm}[H]
  \LinesNumbered
  \KwIn{$E_{K}, (\Delta\In, \Delta\Out), N, p = \pr(\Delta\In, \Delta\Out)$}
  \KwOut{0: \textbf{real} cipher, 1: \textbf{ideal} cipher}
  Initialize counter $T$ with zero\;
  \For{$i = 0, \ldots, N - 1$}{
        $P_{1} \overset{\$}{\gets} \mathbb{F}_{2}^{n}$\;
        $C_{1} \gets E_{K}(P_{1})$\;
        $P_{2} \gets P_{1} \oplus \Delta\In$\;
        $C_{2} \gets E_{K}(P_{2})$\;
        \If{$C_{1} \oplus C_{2} = \Delta\Out$}{
            $T \gets T + 1$\;
        }
  }
  \If{$T\sim \mathcal{N}(\mu = Np, \sigma^{2} = N p (1- p))$}{
    \Return 0\tcp*[r]{real cipher}
  } \Else {
    \Return 1\tcp*[r]{ideal cipher}
  }
\end{algorithm}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Analytical Estimation of Differential Probability}
\vspace{-0.4cm}
\begin{center}
...
\node[] at ($(here) + (0.25, -0.31)$) {$\Delta X_0$};
\draw[next] (AK) -- (R);
\draw[next] (R) -- (DOTS);
\node[] at ($(R) + (0.92, -0.31)$) {$\Delta X_{1}$};
...
\node[right=1cm of R] (DOTS){$\cdots$};
\draw[next] (here) -- (AK);
\node[] at ($(here) + (0.25, -0.31)$) {$\Delta X_{i-1}$};
\draw[next] (AK) -- (R);
\draw[next] (R) -- (DOTS);
\node[] at ($(R) + (0.92, -0.31)$) {$\Delta X_{i}$};
...
\end{center}
\[
\pr\!\left(\Delta X_r = \Delta_r\mid\Delta X_0 = \Delta_0\right)\!\!=\!\!\sum_{\Delta_1, \cdots, \Delta_{r-1}}\!\prod_{i = 1}^{r} \pr(\bm{f}_{i}(X) \oplus \bm{f}_{i}(X \oplus \Delta_{i-1})\!=\!\Delta_{i}).
\]
\end{frame}
...
\begin{frame}{Difference Distribution Table (\ddt) -- I}
  \begin{itemize}
    \item We need a tool to handle the nonlinear operations
  \end{itemize}
    % \begin{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Linear Attack}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Linear Attacks \cite{eurocrypt_Matsui93}}
\begin{columns}[onlytextwidth]
\column[c]{0.6\textwidth}
\resizebox{0.74\textwidth}{!}{
\begin{algorithm}[H]
  \LinesNumbered
  \KwIn{$E_{K}$, Given $N$ distinct plaintext-ciphertext pairs $(P_i, C_i)$, $\bm{c} = \corr(\lambda\In, \lambda\Out)$}
  \KwOut{0: \textbf{real} cipher, 1: \textbf{ideal} cipher}
  Initialize a counter list $V[z] \gets 0$ for $z \in \{0, 1\}$\;
  \For{$t = 0, \ldots, N - 1$}{
        $b_{1} \gets \lambda\In \cdot P_{t}$\;
        $b_{2} \gets \lambda\Out \cdot C_{t}$\;
        $V[b_{1} \oplus b_{2}] \gets V[b_{1} \oplus b_{2}] + 1$\;
  }
  \If{$V[0] \sim \mathcal{N}(\mu_0 = N\frac{1 + \bm{c}}{2}, \sigma_0^2 = \frac{N(1 - c^2)}{4}).$}{
    \Return 0\tcp*[r]{real cipher}
  } \Else {
    \Return 1\tcp*[r]{ideal cipher}
  }
...
\draw[->, dashed] (xp) -- (yp);
\node[below=0.2cm of dc] {\scriptsize \textcolor{tugred}{$\bm{c} = 2\cdot \pr(\bm{\lambda\In}\cdot P \oplus \bm{\lambda\Out}\cdot C  =0) - 1$}}; 
\end{tikzpicture}
\end{overprint}
\end{center}
\end{columns}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Analytical Estimation of Correlation}
\begin{center}
\resizebox{1\textwidth}{!}{
\begin{tikzpicture}[xscale=1,yscale=1,very thin,>=latex]
\coordinate (here) at (0, 0);
...
\coordinate (K0_1) at ($(AK.north) + (-1.15, 1.85)$);
\coordinate (K0_2) at ($(AK.north) + (0, 1.85)$);
\node[] at ($(here) + (0.25, -0.31)$) {$\Gamma_{0}$};
\draw[next] (AK) -- (R);
\draw[next] (R) -- (DOTS);
\node[] at ($(R) + (0.92, -0.31)$) {$\Gamma_{1}$};
\draw[next] (RKSource) -- node[anchor=east] {$K_{1}$} (AK);
\draw[fill opacity = 1, rounded corners=1mm, dashed] ($(here) + (0.78, -0.5)$) rectangle ($(here) + (2.65, 0.55)$) node[below, midway, yshift=-0.5cm] {$\bm{F}_1$};
\coordinate[right=1pt of DOTS] (here);
\draw[next, rounded corners=1mm] (K0_1) -- node[above] {$K$} (K0_2);
...
\node[right=1cm of R] (DOTS){$\cdots$};
\draw[next] (here) -- (AK);
\node[] at ($(here) + (0.25, -0.31)$) {$\Gamma_{i - 1}$};
\draw[next] (AK) -- (R);
\draw[next] (R) -- (DOTS);
\node[] at ($(R) + (0.92, -0.31)$) {$\Gamma_{i}$};
\draw[next] (RKSource) -- node[anchor=east] {$K_{i}$} (AK);
\draw[fill opacity = 1, rounded corners=1mm, dashed] ($(here) + (0.78, -0.5)$) rectangle ($(here) + (2.65, 0.55)$) node[below, midway, yshift=-0.5cm] {$\bm{F}_i$};
\coordinate[right=1pt of DOTS] (here);
...
\coordinate (T3) at ($(AK.north) + (1.5, 2.2)$) {};
\draw[next] (here) -- (AK);
\node[next] at ($(here) + (0.25, -0.31)$) {$\Gamma_{r - 1}$};
\draw[next] (AK) -- (R);
\draw[next] (R) -- (DOTS);
\draw[next] (DOTS) -- (cipher);
\node[xor] (ALK) at (DOTS) {};
\coordinate[above=1.5cm of ALK] (LRKSource);
\node[] at ($(R) + (0.92, -0.31)$) {$\Gamma_{r}$};
\draw[next] (RKSource) -- node[anchor=east] {$K_{r}$} (AK);
\draw[next] (LRKSource) -- node[anchor=east] {$K_{r + 1}$} (ALK);
\draw[fill opacity = 1, rounded corners=1mm, dashed] ($(here) + (0.78, -0.5)$) rectangle ($(here) + (2.65, 0.55)$) node[below, midway, yshift=-0.5cm] {$\bm{F}_r$};
\draw[rounded corners=1mm] (T1) -- (T2) -- (T3) -- (T4) -- cycle;
...
\end{center}
\[
\corr(\Gamma_{0}, \Gamma_{r + 1}) \approx (-1)^{\left(\Gamma_{0} \cdot K_{1} \oplus \cdots \oplus \Gamma_{r} \cdot K_{r + 1}\right)} \prod_{i = 1}^{r} \corr_{\bm{f}_i}(\Gamma_{i - 1}, \Gamma_{i}).
\]
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Minimum Number of Differentially Active S-boxes in \cipher{AES}}
\begin{figure}
\centering
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Security of \cipher{AES} Against Differential Attacks}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Boomerang Attack}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]{Boomerang Distinguishers \cite{fse_Wagner99}}
\setlength{\columnsep}{0.5em} % Adjust the spacing between columns
\begin{columns}[onlytextwidth]
\column[c]{0.6\textwidth}
\resizebox{0.74\textwidth}{!}{
\begin{algorithm}[H]
  \LinesNumbered
  \KwIn{$E_{K}, (\Delta, \nabla), N, P = \pr(P_{3} \oplus P_{4} = \Delta)$}
  \KwOut{0: \textbf{real} cipher, 1: \textbf{ideal} cipher}
  Initialize counter $T$ with zero\;
  \For{$i = 0, \ldots, N - 1$}{
    $P_{1} \overset{\$}{\gets} \mathbb{F}_{2}^{n}; ~ P_2 = P_1 \oplus \Delta$\;
    $C_{1} \gets E_{K}(P_{1}), \quad C_{2} \gets E_{K}(P_{2})$\;
    $C_{3} \gets C_{1} \oplus \nabla, \quad C_{4} \gets C_{2} \oplus \nabla$\;
    $P_{3} \gets D_{K}(C_{3}), \quad P_{4} \gets D_{K}(C_{4})$\;
    \If{$P_{3} \oplus P_{4} = \Delta$}{
      $T \gets T + 1$\;
    }
  }
  \If{$T\sim \mathcal{N}(\mu = NP, \sigma^{2} = N P (1- P))$}{
...
\draw[<->, dashed, tugred] (p1) -- node[above] {$\Delta$} (p2);
\draw[<->, dashed, tugred] (x1) -- node[below] {} (x2);
\draw[->] (p1) -- (e0l1) -- (x1);
\draw[->] (x1) -- (e1l1) -- (c1);
\draw[->] (p2) -- (e0l2) -- (x2);
\draw[->] (x2) -- (e1l2) -- (c2);
...
\draw[<->, dashed, mach] (x1) -- node[above] {} (x3);
\draw[<->, dashed, mach] (x2) -- node[below] {} (x4);
\draw[->] (c3) -- (e1r1) -- (x3);
\draw[->] (c4) -- (e1r2) -- (x4);
\node[right=\nablao of p1] (p3) {$P_{3}$};
\node[right=\nablao of p2] (p4) {$P_{4}$};
...
\draw[<->, dashed, tugred] (x3) -- node[above] {} (x4);
\draw[<->, dashed, tugred] (p3) -- node[above] {$\Delta$} (p4);
\end{tikzpicture}}
\end{flushright}
\end{overprint}
\end{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Probability of Boomerang Distinguishers \cite{fse_Wagner99}}
\begin{columns}
\column[c]{0.4\textwidth}
\centering
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Generalized BCT Framework -- I}
\begin{columns}
\column[c]{0.5\textwidth}
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Differential-Linear (DL) Attack I \cite{dl_crypto_LangfordH94}}
\vspace{-0.4cm}
\begin{columns}
\column[c]{0.7\textwidth}
\resizebox{0.74\textwidth}{!}{
\begin{algorithm}[H]
  \LinesNumbered
  \KwIn{$E_{K}, (\Delta, \lambda), N, \bm{c} = \corr(\Delta, \lambda)$}
  \KwOut{0: \textbf{real} cipher, 1: \textbf{ideal} cipher}
  Initialize a counter list $V[z] \gets 0$ for $z \in \{0, 1\}$\;
  \For{$i = 0, \ldots, N - 1$}{
        $P_{1} \overset{\$}{\gets} \mathbb{F}_{2}^{n}$\;
        $b_{1} \gets \lambda \cdot E_{K}(P_{1})$\;
        $P_{2} \gets P_{1} \oplus \Delta$\;
        $b_{2} \gets \lambda \cdot E_{K}(P_{2})$\;
        $V[b_{1} \oplus b_{2}] \gets V[b_{1} \oplus b_{2}] + 1$\;
  }
  \If{$V[0] \sim \mathcal{N}(\mu = N\frac{1 + \bm{c}}{2}, \sigma^{2} = N\frac{1- \bm{c}^{2}}{4})$}{
    \Return 0\tcp*[r]{real cipher}
  } \Else {
    \Return 1\tcp*[r]{ideal cipher}
  }
...
\begin{tikzpicture}[bc/.style={draw,minimum height=1cm, minimum width=.7cm, rounded corners=2pt}]
  ...
  \draw[dashed, <->] (Pp1) -- node[tugred, above] (Pm) {$\Delta$} (Pp2);
  \draw[dashed, <->] (Xp1) -- node[tugred, above] (Xm) {} (Xp2);
  \draw[dashed, ->] (Pm)++(0, -\dy/4) -- node[right]{$p$} (Xm);
  \draw (Xp1) ++(0.7,-0.2) node[mach] (Gm1) {}
        (Xp2) ++(-0.7,-0.2) node[mach] (Gm2) {};
  \foreach \i/\labelside in {1/left, 2/right} {
    \draw (Gm\i|-C\i) node[mach] (Go\i) {$\lambda$};
    \draw[dashed, ->] (Gm\i) -- node[\labelside] {$q$} (Go\i);
    }
  \draw[dashed, ->, gray] (Go1) to[out=90, in=180] ($(Xm)+(0, -0.4)$) to[out=0, in=90] (Go2);
  \coordinate (Gmidd) at ($(Go1)!0.5!(Go2)$);
  \node[below=0.2cm of Gmidd] {\scriptsize \textcolor{tugred}{$\bm{c} = 2\!\cdot\!\pr(\bm{\lambda}\cdot C_{1}\!\oplus\!\bm{\lambda}\cdot C_{2}\!=\!0)\!-\!1$}}; 
\end{tikzpicture}
\end{center}
\end{columns}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Differential-Linear Attacks}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Bit-Wise Model for Finding ID/ZC/Integral Distinguishers}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{CP Model for Deterministic Bit-Wise Trails}
\vspace{-0.5cm}
\begin{itemize}
\small
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Example: ID/ZC Distinguishers for 5 Rounds of \cipher{Ascon} \cite{zeroplus_cryptoeprint_2023_1701}}
\begin{columns}
\column[c]{0.50\textwidth}
\centering
\begin{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Generic and Common Techniques in Symmetric-Key Attacks}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Guess-and-Determine: A Common Step in Most Attacks}
...
\begin{frame}{Knowledge Propagation}
  \begin{columns}
    ...
    \begin{figure}
      ...
      \only<1>{\includegraphics[width=1\textwidth, trim=0 0 0 0, clip]{./figures/KP1.pdf}}
      \only<2>{\includegraphics[width=1\textwidth, trim=0 0 0 0, clip]{./figures/KP2.pdf}}
      \only<3>{\includegraphics[width=1\textwidth, trim=0 0 0 0, clip]{./figures/KP3.pdf}}
      \only<4>{\includegraphics[width=1\textwidth, trim=0 0 0 0, clip]{./figures/KP4.pdf}}
      \only<5>{\includegraphics[width=1\textwidth, trim=0 0 0 0, clip]{./figures/KP5.pdf}}
      \only<6>{\includegraphics[width=1\textwidth, trim=0 0 0 0, clip]{./figures/KP6.pdf}}
      \end{overlayarea}
    \end{figure}
  \end{columns}
\end{frame}
...
\begin{center}
  \includegraphics[width=0.7\textwidth]{./figures/aes_1_round_gd_dg.pdf}
\end{center}
\vspace{-0.2cm}
\scriptsize
\centering
