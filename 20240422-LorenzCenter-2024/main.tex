...
\date{Lorentz Center, 22 April, 2024 - Leiden, Netherlands}
%\institute{IAIK}
...
where $\ldlct_b(\Delta\In, \lambda\In, \lambda\Out) = \{x\in \mathbb{F}_{2}^{n}:~ \lambda\Out \cdot S(x) = \lambda\Out \cdot S(x \oplus \Delta\In) \text{~and~} \lambda\In \cdot \Delta\In = b \}$.
  \end{block}
...
\begin{tikzpicture}
  ...
  \draw[->] (D2) -- (L3);
\end{tikzpicture}
\[\ddlct(\Delta\In, \lambda\Out) = \sum_{\Delta\Mid}\sum_{\lambda\Mid}  \udlct\left(\Delta\In, \Delta\Mid, \lambda\Mid\right) \cdot \ldlct\left(\Delta\Mid, \lambda\Mid, \lambda\Out\right)\]
\end{center}
\column[c]{0.25\textwidth}
\begin{tikzpicture}
...
\begin{itemize}
\item $\ddlct(\Delta\In, \lambda\Out) = \sum_{\Delta\Mid}\sum_{\lambda\Mid}  \udlct\left(\Delta\In, \Delta\Mid, \lambda\Mid\right) \cdot \ldlct\left(\Delta\Mid, \lambda\Mid, \lambda\Out\right)$
\end{itemize}
\begin{align*}
